...
extern "C" int spp_handle_init(void* arg1, void* arg2)
{
	...
	if(iret != 0)
	{
        base->log_.LOG_P_PID(LOG_ERROR, "INITERROR:%d\n", iret);
	}
...
extern "C" int spp_handle_process(unsigned flow, void* arg1, void* arg2)
{
	...
	std::string request(blob->data);
	std::string	response;
	//base->log_.LOG_P_PID(LOG_DEBUG, "spp_handle_process, data:%s\n", tmp);
	...
	int ret = process_message(request, response);
	
	blob_type response_blob;
	response_blob.len = response.size() + 1;
	response_blob.data = new char[response.size() + 1];
	strncpy(response_blob.data,response.c_str(),response.size());
	response_blob.data[response.size() + 1] = '\0';
	
    //echo logic
    //int ret = commu->sendto(flow, arg1, arg2 );
    ret = commu->sendto(flow, &response_blob, arg2 );
	
	if(response_blob.data != NULL)
	{
		free(response_blob.data);
	}
    if (ret != 0)
    {
